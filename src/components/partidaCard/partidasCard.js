...
export default function PartidasCard({ endereco }) {
  const [data, setData] = useState([]);
  const [dataQuadra, setDataQuadra] = useState([]);
  const [dataQuadraEndereco, setDataQuadraEndereco] = useState([]);
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `${baseUrl}/partida/municipio/${endereco}`
        );
        setDataQuadraEndereco(response.data);
        console.log("Dados do back-end", dataQuadraEndereco);
      } catch (e) {
        console.log("Erro ao buscar dados do back-end", e);
      }
    };
    if (endereco) fetchData(); // Adiciona esta linha para evitar chamadas desnecessárias
  }, [endereco]);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.item}
      onPress={() => navigation.navigate("DescricaoJogo", { itemId: item.id })}
    >
      <Text style={styles.title}>{item.quadra.nome}</Text>
      <Icon
        name={item.quadra.categoria?.descricao}
        size={100}
        color="white"
        style={styles.icon}
      />
      ...
      <Text style={styles.participants}>
        {new Set(item.usuario).size}/{item.numeroJogadores}
      </Text>
